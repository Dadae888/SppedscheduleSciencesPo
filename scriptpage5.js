...
const timetable = document.getElementById('timetable2'); 
...
async function loadEvents() {
    const res = await fetch('/api/page5events');
    const page5events = await res.json();
	console.log(page5events); 
    renderTimetable(page5events);
}

function renderTimetable(page5events) {
    timetable.innerHTML = '';
...
async function openAttendersList(eventData) {
    ...
    document.getElementById('joinBtn').addEventListener('click', async () => {
        ...
        await fetch('/api/attenderslist2', {
            method: 'POST',
